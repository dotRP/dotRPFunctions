	...
	while true do 
        ...
        then
            if math.floor(currentcarspeed*Config.Speedo_Math) >= Config.Class_Compacts_Maxspeed then      
				...
				SetVehicleMaxSpeed(GetVehiclePedIsIn(PlayerPedId(), false), setspeed-1)
			end
        ...
        then
            ...
            Citizen.Wait(2000) -- Addded a wait here, so its not trying to do it each frame, might disable if weridness  
	end
end)